...
total_customers as (
  select format_date('%W', date) as week,
...
orders as (
  select distinct o.pd_customer_uuid, 
    date_diff(o.created_date_local, c.date, week) as week_number,
...
, retentions as (
  select format_date('%W', c.date) as week,
    o.week_number,
...
select r.week, t.total_customers, r.week_number, r.num_users as users_retained, r.num_users / t.total_customers as retention_rate
from retentions r
